...
def parse_eval_args():
    ...
    parser.add_argument("--debug_prompt", action='store_true', help="If set, only process one sample per dataset for prompt sanity check")
...
def main():
    ...
    for dataset_idx, (dataset_name, task_config) in enumerate(dataset_configs.items()):
        ...
        if dist.is_initialized():
            dist.barrier()
        print_master(f"--- Checking {dataset_name} ---")
    ...
    output_name = f"prompts_{os.path.basename(data_args.dataset_config).replace('.yaml', '')}_unified"
    if data_args.apply_chat_template:
        output_name += "_chat"
    if data_args.query_description_dir:
        output_name += "_qry_desc"
    if data_args.target_description_dir:
        output_name += "_tgt_desc"
