...
from src.model_utils import (LLAVA_NEXT, 
                             QWEN2_VL, 
                             PHI3V, 
                             get_backbone_name, 
                             print_master, 
...
from .model_utils import get_visual_token_ids, load_processor
...
class MMEBModel(nn.Module):
    ...
    def __init__(self,
                 ...
                 processor,
                 learnable_queries: Optional[nn.Parameter] = None,
                 pooling_module = None
                 ):
        super().__init__()
    ...
    def encode_input(self, input, do_sft=False):
        input = batch_to_device(input, self.encoder.device)
        intra_bs = self.model_config.intra_llm_batch_size
        if not self.training and do_sft:
        # if False: # debug
                # genearte then encode
                with torch.no_grad():
                    results = self.encoder.generate(**{
                        'input_ids': input['input_ids'],
                        'attention_mask': input['attention_mask'],
            ...
            if self.model_config.freeze_base_model:
                with torch.no_grad():
                    ...
                    hidden_states = results.hidden_states[-1].detach()
            else:
                results = self.encoder(**input, return_dict=True, output_hidden_states=True)
                hidden_states = results.hidden_states[-1]

        pooled_output = self._pooling(hidden_states, input['attention_mask'], input, results, do_sft=do_sft)
        return pooled_output, results

    def _pooling(self, last_hidden_state, attention_mask, input, results, do_sft=False, is_target=False):
        if self.pooling_module_type in {'last', 'eos', 'mlp'}:
            ...
            if do_sft and not self.training:
                ...
                if left_padding:
                    if self.model_config.do_sft and "system" in input['texts'][0] and "user" in input['texts'][0]:
            ...
            # TODO: try modify the hidden states to only include tokens after the last image token
            # then pad to the same length in the batch
            ...
            if self.model_config.mask_visual_tokens_for_pooling:
                ...
                for visual_token_id in self.visual_token_ids:
                    attention_mask[input['input_ids'] == visual_token_id] = 0
            if self.pooling_module_type == "patched_pooler":
                reps = self.pooling_module(results.hidden_states, attention_mask)
            elif self.pooling_module_type == "truncated_self_pooler":
                # Truncated self pooling
                reps = self.pooling_module(results.hidden_states, padding_mask=attention_mask)
        else:
    ...
    def build(cls, model_args: ModelArguments, **kwargs):
        ...
        if model_args.lora:
            assert model_args.sft_checkpoint_path is None, "lora is not supported with checkpoint_path"
        ...
        learnable_queries = None
        if model_args.learnable_queries:
            print_master(f'Initializing learnable queries')
            learnable_queries = nn.Parameter(torch.zeros(model_args.learnable_queries, base_model.config.hidden_size))
    ...
    def load(cls, model_args: ModelArguments, **kwargs):
        ...
        if model_args.pooling_module not in {'last', 'eos'}:
            ...
            pooler_state_dict = torch.load(os.path.join(checkpoint_path, 'pooling_module', 'pytorch_model.bin'), map_location='cpu')
            ...
            if model_args.pooling_module == 'attention_pooler':
                pooling_config = AttentionPoolingConfig.from_pretrained(
                    os.path.join(checkpoint_path, 'pooling_module'),
                    trust_remote_code=True
                )
    ...
    def save(self, output_dir: str):
        ...
        if self.pooling_module is not None:
            self.pooling_module.save_pretrained(os.path.join(output_dir, 'pooling_module'))
    ...
    def forward(self, qry: Dict[str, Tensor] = None, tgt: Dict[str, Tensor] = None, *args, **kwargs):
        ...
        if qry is not None:
            qry_reps, qry_res = self.encode_input(qry, do_sft=self.model_config.do_sft)
        else:
            qry_reps = None
        ...
        if self.model_config.do_sft and qry_res is not None:
            loss_dict['sft_loss'] = qry_res.loss
        
        # if self.model_config.do_sft_target and tgt_res is not None:
...
# AutoModel.register(AttentionPoolingConfig, AttentionPoolingModule)
# AttentionPoolingModule.register_for_auto_class("AutoModel")
