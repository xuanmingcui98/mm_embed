...
class GuiEvalDatasetProcessor(MMEBV2EvalDatasetProcessor):
    ...
    def _load_hf_dataset(self):
        ...
        subset_name = self.dataset_config.get("subset_name")
        return load_dataset(dataset_name, subset_name, split=f"{dataset_split}")
        ...
        for qry_text, qry_image_path, cand_id, retrieval_type in \
            ...
            if self.apply_chat_template:
                if text_keyname == 'qry_text':
                    qry_text = self.format_text_for_chat_template(qry_text, 
                                                                  image_path=qry_image_path, 
                                                                  description=self.query_descriptions[(orig_qry, qry_image_path)] if self.query_descriptions else None,
                                                                  add_generation_prompt=self.model_args.do_sft_query)
                else:
                    qry_text = self.format_text_for_chat_template(qry_text, 
                                                                  image_path=qry_image_path, 
                                                                  description=self.target_descriptions[(orig_qry, qry_image_path)] if self.target_descriptions else None,
