...
class VidoReDatasetProcessor(VideoDatasetProcessor):
    ...
    def _process_one_sample(self, idx, batch_dict, *args, **kwargs):
        ...
        if self.data_args.hard_negative_dir:
            ...
            neg_text = [x['answer'] for x in hard_negative_samples]
...
class VidoReSFTDatasetProcessor(VideoSFTDatasetProcessor):
    def __init__(self, *args, **dataset_config):
        super().__init__(DATASET_PARSER_NAME, *args, **dataset_config)
    ...
    def _process_one_sample(self, idx, batch_dict, *args, **kwargs):
        ...
        answer = self.query_descriptions.get((batch_dict['id'][idx],))

        return {"prompt": query, 
                "image": {"bytes": [image_bytes], "paths": [path]}, 
                "answer": answer}