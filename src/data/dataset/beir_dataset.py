...
from src.data.dataset.base_pair_dataset import AutoPairDataset, add_metainfo_hook, MULTIMODAL_FEATURES, BaseDatasetProcessor
...
class BEIRDatasetProcessor(BaseDatasetProcessor):
    ...
    # @xuanming: currently not working. Seems not used in current vlm2vec v2
    def __init__(self, model_args, data_args, training_args, dataset_config=None, **kwargs):
        super().__init__(DATASET_PARSER_NAME, model_args, data_args, training_args, dataset_config=dataset_config, **kwargs)
        

    @add_metainfo_hook
    def data_prepare(self, batch_dict, dataset_name=None, num_hardneg=0,
    ...
    def load(self, model_args, data_args, training_args, dataset_name, file_path,
        ...
        kwargs['global_dataset_name'] = f'{self.dataset_parser_name}/{dataset_name}'
        # create dataset
        ...
        dataset = dataset.map(lambda x: self.data_prepare(x, dataset_name=dataset_name, num_hardneg=num_hardneg,
