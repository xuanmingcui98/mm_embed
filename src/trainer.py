...
class MMEBTrainer(Trainer):
    def __init__(self, *args, **kwargs):
        ...
        self.training_args = kwargs.pop("training_args", None)
        self.max_length = self.data_args.max_len
        self.apply_chat_template = self.data_args.apply_chat_template
        self.apply_chat_template_target = self.data_args.apply_chat_template_target
        self.do_sft = kwargs['model'].model_config.do_sft
        self.do_sft_target = kwargs['model'].model_config.do_sft_target
        self.do_cl = kwargs['model'].model_config.do_cl

        super(MMEBTrainer, self).__init__(*args, **kwargs)
        self.is_ddp = dist.is_initialized()
        self.processor = functools.partial(process_vlm_inputs_fns[self.args.model_backbone], processor=self.processing_class, max_length=self.max_length, use_default_system_prompt=self.data_args.use_default_system_prompt)
    ...
    def compute_loss(self, model, inputs, *args, **kwargs):
        ...
        if "input_ids" not in qry_inputs:
            qry_inputs = self.processor(qry_inputs, 
                                        apply_chat_template=self.apply_chat_template, 
                                        do_sft=self.do_sft, 
                                        # add_description=self.data_args.descriptions is not None or self.data_args.description_dir is not None,
                                        add_generation_prompt=False)
        if tgt_inputs:
            if "input_ids" not in tgt_inputs:
                tgt_inputs = self.processor(tgt_inputs, 
                                            apply_chat_template=self.apply_chat_template_target, 
                                            do_sft=self.do_sft_target, 
                                            # add_description=self.do_sft_target or self.data_args.target_description_dir is not None,
                                            add_generation_prompt=False)
    ...
    def _save(self, output_dir: Optional[str] = None, state_dict=None):
        ...
        # encoder_state_dict = {"encoder." + k if not k.startswith("encoder.") else k : v for k, v in encoder_state_dict.items()}
        encoder_state_dict = {k.replace("encoder.", "") : v for k, v in encoder_state_dict.items()}
        self.model.encoder.save_pretrained(
            output_dir, state_dict=encoder_state_dict, safe_serialization=False #self.args.save_safetensors
        )
    ...
    def _inner_training_loop(
        ...
        # check if accelerator ddp is initiated properly
        ...
        # default loss as torch.tensor(0.0).to(args.device)
...
class GradCacheTrainer(MMEBTrainer):
    ...
    def _save(self, output_dir: Optional[str] = None, state_dict=None):
        ...
        self.model.encoder.save_pretrained(
            output_dir, state_dict=state_dict, safe_serialization=False, #self.args.save_safetensors
        )
...
class GradCacheLateProcessTrainer(MMEBTrainer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        loss_fn_cls = DistributedContrastiveLoss if self.is_ddp else SimpleContrastiveLoss
        loss_fn = loss_fn_cls(temperature=self.model.temperature)
        process_fn = functools.partial(process_vlm_inputs_fns[self.args.model_backbone], processor=self.processing_class, max_length=self.max_length, use_default_system_prompt=self.data_args.use_default_system_prompt)
        ...
        self.gc = GradCache(
            ...
            fp16=self.args.fp16,
            scaler=self.scaler if self.args.fp16 else None,
            cl_loss_scalar=self.training_args.cl_loss_scalar,
            training_args=self.training_args,
            data_args=self.data_args,
        )

        self.gc.apply_chat_template = self.apply_chat_template
        self.gc.apply_chat_template_target = self.apply_chat_template_target
        self.gc.do_sft = self.do_sft
        self.gc.do_sft_target = self.do_sft_target
        self.gc.data_args = self.data_args
        self.gc.do_cl = self.do_cl
        self.gc.sft_loss_scalar = self.training_args.sft_loss_scalar
    ...
    def training_step(self, model, inputs, *args, **kwargs) -> torch.Tensor:
        ...
        # total_loss = total_loss / self._dist_loss_scale_factor
        # for k, v in loss_dict.items():
        #     loss_dict[k] = v / self._dist_loss_scale_factor
