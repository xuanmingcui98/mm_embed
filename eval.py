...
def main():
    ...
    categories = {
        "classification": {"ImageNet-1K", "N24News", "HatefulMemes", "VOC2007", "SUN397", "Place365", "ImageNet-A", "ImageNet-R", "ObjectNet", "Country-211"},
    ...
    ood = {
        "classification": {"Place365", "ImageNet-A", "ImageNet-R", "ObjectNet", "Country-211"},
    ...
    for idx, subset in enumerate(data_args.subset_name):
        ...
        if not os.path.exists(encode_qry_path) :
            ...
            with torch.no_grad():
                ...
                for batch in tqdm(eval_qry_loader):
        ...
        if not os.path.exists(encode_tgt_path):
            ...
            with torch.no_grad():
                for batch in tqdm(eval_tgt_loader):
                    batch = batch_to_device(batch, training_args.device)
                    with torch.autocast(enabled=True, dtype=torch.bfloat16, device_type="cuda"):
