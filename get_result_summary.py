...
def get_result_summary(dir):
    ...
    for experiment in EXPERIMENTS:
        ...
        for modality in modalities:
                        ...
                        try:
                            with open(score_file_path, "r") as f:
                                ...
                                if modality in {"image", "video"}:
                                    current_experiment_scores[modality][dataset_name_from_file] = score_data['hit@1']
                                    all_scores.append(score_data['hit@1'])
                                    current_scores.append(score_data['hit@1'])
                                elif modality == "visdoc":
                                    all_scores.append(score_data['ndcg_linear@5'])
                                    current_scores.append(score_data['ndcg_linear@5'])
                                    current_experiment_scores[modality][dataset_name_from_file] = score_data['ndcg_linear@5']
            ...
            current_experiment_scores[modality]['overall'] = sum(current_scores) / len(current_scores)
        ...
        current_experiment_scores['all_scores'] = sum(all_scores) / len(all_scores) 

        # get one level up directory

        output_json_path = os.path.join(os.path.dirname(base_path), f"summary.json") if args.output_path is None else args.output_path
        try:
        ...
        for modality in modalities:
            ...
            for dataset_name in modality2dataset.get(modality, []):
                ...
                elif isinstance(score_info, (str, float)):
        ...
        for modality in modalities:
            ...
            for dataset_name in modality2dataset.get(modality, []):
                ...
                elif isinstance(score_info, float):
                    collected_metric_values.append(score_info)
                else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output_path', type=str, default=None, help='Path to save the summary JSON report (default: summary.json in the experiment directory)')
