...
class GradCache:
    ...
    def forward_no_grad(
        ...
        with torch.no_grad():
            for x in model_inputs:
                ...
                y, logits = self.model_call(model, x)
                model_reps.append(self.get_reps(y))
                model_logits.append(logits['target_logits'] if logits['target_logits'] is not None else logits['query_logits'])

        # concatenate all sub-batch representations
        model_reps = torch.cat(model_reps, dim=0)
        model_logits =  torch.cat(model_logits, dim=0)

        return model_reps, rnd_states, model_logits

    def build_cache(self, *reps: Tensor, 
                    query_logits = None, 
                    target_logits = None, 
                    query_labels = None, 
                    target_labels = None, **loss_kwargs) -> [List[Tensor], Tensor]:
        """
    ...
    def cache_step(
                ...
                for key2val_dict in arg_group:
                    ...
                    for arg_key, arg_val in key2val_dict.items():
                        _key2val_dict[arg_key] = self.process_fn(arg_val, 
                                                                 ...
                                                                 do_sft=do_sft, 
                                                                 add_answer=self.data_args.descriptions is not None or self.data_args.description_dir is not None)
        ...
        for idx, model, x in enumerate(zip(self.models, model_inputs)):
            model_reps, rnd_states, model_logits = self.forward_no_grad(model, x)
            all_reps.append(model_reps)
            all_rnd_states.append(rnd_states)
            if idx == 0:
                # query
                if self.do_sft:
                    all_query_logits.append(model_logits)
            else:
                # target
                if self.do_sft_target:
                    all_target_logits.append(model_logits)
