...
class GradCache:
    ...
    def __init__(
        ...
        self.cl_loss_scalar = cl_loss_scalar
    ...
    def model_call(self, model: nn.Module, model_input, **kwargs):
        """
        ...
        with autocast() if self.fp16 else nullcontext():
            if isinstance(model_input, Tensor):
                ...
                return model(*model_args, **model_kwargs, **kwargs)
            else:
    ...
    def build_cache(self, *reps: Tensor, **loss_kwargs) -> [List[Tensor], Tensor]:
        ...
        with autocast() if self.fp16 else nullcontext():
            loss = self.compute_loss(*reps, **loss_kwargs) * self.cl_loss_scalar
        ...
        return cache, loss.detach() / self.cl_loss_scalar
    ...
    def forward_backward_sft(
        ...
        for x, sync_context in zip(model_inputs, sync_contexts):
            with sync_context():
                ...
                loss = loss / len(model_inputs)  # average loss across sub-batches
                loss = loss * self.training_args.sft_loss_scalar
    ...
    def cache_step(
        ...
        if self.process_fn:
            ...
            _model_inputs = []
            for idx, arg_group in enumerate(model_inputs):

                if idx == 0:
                    # query
                    do_sft = self.do_sft
                    apply_chat_template = self.apply_chat_template
                    system_prompt_key = 'qry_system_prompt'
                    ...
                    system_prompt_key = 'tgt_system_prompt'
                    add_description = self.data_args.target_description_dir is not None
                _arg_groups = []
        ...
        for idx, (model, x) in enumerate(zip(self.models, model_inputs)):
            if idx == 0 and self.do_sft:
                # query
                query_sft_loss = self.forward_backward_sft(model, x, no_sync_except_last=no_sync_except_last, key='qry_sft_loss')
                loss_dict['qry_sft_loss'] = query_sft_loss / self.training_args.sft_loss_scalar
            elif idx == 1 and self.do_sft_target:
                target_sft_loss = self.forward_backward_sft(model, x, no_sync_except_last=no_sync_except_last, key='tgt_sft_loss')
                loss_dict['tgt_sft_loss'] = target_sft_loss / self.training_args.sft_loss_scalar
